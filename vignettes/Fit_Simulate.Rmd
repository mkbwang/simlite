...
count_data <- assay(gut_experiment_subset)
```
...
```{r}
pois_params <- fitdistr_mat(count_mat=count_data_subset, dist="pois", cutoff=0.97)
head(pois_params)
```
...
```{r}
zipois_params <- fitdistr_mat(count_mat=count_data_subset, dist="zipois", cutoff=0.97)
head(zipois_params)
```
...
```{r}
nb_params <- fitdistr_mat(count_mat=count_data_subset, dist="nb", cutoff=0.97)
head(nb_params)
```
...
```{r}
zinb_params <- fitdistr_mat(count_mat=count_data_subset, dist="zinb", cutoff=0.97)
head(zinb_params)
```
...
```{r}
zigamma_params <- fitdistr_mat(count_mat=count_data_subset, dist="zigamma", cutoff=0.97)
head(zigamma_params)
```
...
```{r}
zilnorm_params <- fitdistr_mat(count_mat=count_data_subset, dist="zilnorm", cutoff=0.97)
head(zilnorm_params)
```
